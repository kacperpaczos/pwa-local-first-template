...
/**
 * Resolve one LWW field between two replicas, in two descending levels:
 * 1. Higher Lamport clock wins outright.
 * 2. On a Lamport tie (concurrent independent edits), fall back to a value
 *    comparison. This step must be a pure, symmetric function of the two
 *    values being compared, not favor either side by role — an asymmetric
 *    "remote wins ties" rule makes two peers merging the same pair pick
 *    opposite winners (each calls the *other* side "remote"), so their
 *    state permanently diverges instead of converging.
 *
 * Wall-clock `updated_at` is deliberately NOT consulted: Lamport ties are
 * common after independent offline edits, and breaking them with
 * unsynchronized device clocks makes the merge outcome depend on clock
 * skew. Deterministic value order is arbitrary but stable on every replica.
 */
