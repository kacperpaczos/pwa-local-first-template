...
**Quarantine, not rejection.** `ingest()` validates an incoming op's signature and chain position (`ok | gap | duplicate | fork`) before ever looking at its payload; a payload that fails to decode or fails to fold into an existing note (`materializeNoteOps`, `shared/store/materialize.ts`) is quarantined per-op — logged with a reason, excluded from future materialization passes — while every other op keeps flowing. One corrupt op can no longer wedge the whole pipeline.

**Real GC coverage** (`shared/sync/coverage.ts`, `shared/sync/engine.ts#isOpCovered`): a tombstone is hard-deletable only once every other device the engine has ever observed (via a head announcement or an ack) has acked the deleting op's `(device, seq)` to at least that height. A device with no known peers is *vacuously* covered — a standalone or not-yet-paired device isn't blocked from ever cleaning up its own tombstones — but once a peer is known, its ack is required before GC proceeds. The 90-day retention window remains a backstop for a peer that goes permanently offline after being observed once.
